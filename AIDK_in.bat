...
powershell.exe -ExecutionPolicy Bypass -NoExit -Command "$ErrorActionPreference = 'Stop'; try { & '.\.venv\Scripts\Activate.ps1'; pip install --upgrade pip wheel; pip install setuptools-82.0.1-py3-none-any.whl; pip install PySide6 requests; pip install huggingface_hub-0.36.2-py3-none-any.whl; pip install llama_cpp_python-0.1.66+cu121-cp311-cp311-win_amd64.whl; pip install torchvision-0.21.0-cp311-cp311-win_amd64.whl; Write-Host ''; Write-Host 'Installation complete. Run AIJDK.bat to start.' -ForegroundColor Green } catch { Write-Host ''; Write-Host \"ERROR: $_\" -ForegroundColor Red; Write-Host 'Installation failed. See above for details.' -ForegroundColor Red }"
